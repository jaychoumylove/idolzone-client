			...
			<view class="item" v-for="(item,index) in list" :key="index">
				...
				<view class="count">
					<btnComponent type="default">
						<view class="flex-set" style="padding: 10upx 40upx;" @tap="join(item.father_uid,item.son_uid,item.son.nickname)">操作</view>
					</btnComponent>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			join(fid, uid, nickname) {
				this.$app.modal(`${nickname} 想要拜师`, () => {
					this.$app.request('father/applyDeal', {
						fid,
						uid,
						status: 1
					}, res => {
			...
			loadData() {
				this.$app.request('father/applyList', {
