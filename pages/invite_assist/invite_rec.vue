			...
			<view class="right-content">
				<view class="earn">
					<view class="right-item">
						<block v-if="item.reward.type == 'currency'">
							<block v-if="item.reward.key == 'coin'">
							...
							<block v-if="item.reward.key == 'flower'">
							...
							<block v-if="item.reward.key == 'stone'">
							...
							<block v-if="item.reward.key == 'trumpet'">
						...
						<block v-else-if="item.reward.type == 'hot'">
						...
						<block v-else>
							<image v-if="item.reward.img" :src="item.reward.img" mode="widthFix"></image>
						</block>
						<view class="add-count add" v-if="item.reward.number > 0">+{{$app.formatNumber(item.reward.number || 0)}}</view>
						<view class="add-count" v-else>{{$app.formatNumber(item.reward.number || 0)}}</view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			getLog() {
				if (this.end) return;
				this.$app.request(this.$app.API.INVITE_SETTLE_REC, {
					page: this.page,
					size: this.size
				}, res => {
