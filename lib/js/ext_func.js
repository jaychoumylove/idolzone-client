...
export default {
	...
	checkUpdate() {
		...
		plus.runtime.getProperty(plus.runtime.appid, widgetInfo => {
			...
			this.request('app/update', {
				...
				if (currentVer == resultVer) return
				// 有更新
	...
	// 充值开关
	chargeSwitch() {
		if (this.getData('platform') == 'MP-WEIXIN') {
			if (this.getData('sysInfo').platform == 'ios') {
				return this.getData('config').ios_switch
			} else {
				return this.getData('config').android_switch
			}
		} else if (this.getData('platform') == 'APP') {
			if (this.getData('sysInfo').platform == 'ios') {
				return this.getData('config').ios_switch_app
			} else {
				return this.getData('config').android_switch_app
			}
		}
	},
}
